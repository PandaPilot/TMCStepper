...
TMC2208Stepper::TMC2208Stepper(uart_inst_t *SerialPort, float RS, uint8_t addr) : TMCStepper(RS),
																				  slave_address(addr)
{
	HWSerial = SerialPort;
	defaults();
...
TMC2208Stepper::TMC2208Stepper(uart_inst_t *SerialPort, float RS, uint8_t addr, uint16_t mul_pin1, uint16_t mul_pin2) : TMC2208Stepper(SerialPort, RS)
{
...
__attribute__((weak)) int TMC2208Stepper::available()
{
	int out = 0;
	if (HWSerial != nullptr)
	{
		if (uart_is_readable(HWSerial))
		{							   // Correct way to check if data is available
			out = uart_getc(HWSerial); // Read a character from UART
		}
		else
		{
			out = -1; // Or handle the case when no data is available
		}
	}
...
__attribute__((weak)) void TMC2208Stepper::preWriteCommunication()
{
	if (HWSerial != nullptr)
	{
		if (sswitch != nullptr)
...
__attribute__((weak)) void TMC2208Stepper::preReadCommunication()
{
		if (HWSerial != nullptr)
	{
		if (sswitch != nullptr)
...
TMC2208Stepper::serial_read()
{
	int16_t out = 0;
		...
		if (HWSerial != nullptr)
	{
		out = uart_getc(HWSerial);  // Pico SDK equivalent for reading a character from UART
	}
...
__attribute__((weak))
uint8_t
TMC2208Stepper::serial_write(const uint8_t data)
{
    int out = 0;

    if (HWSerial != nullptr) {
        uart_putc(HWSerial, data);  // Write the single byte to the hardware UART.
        return 1;  // Return the number of bytes written (similar to Arduino's write).
    }

    return out;
}

__attribute__((weak)) void TMC2208Stepper::postWriteCommunication() {}

__attribute__((weak)) void TMC2208Stepper::postReadCommunication(){}
...
uint64_t TMC2208Stepper::_sendDatagram(uint8_t datagram[], const uint8_t len, uint16_t timeout)
{
	...
	do
	{
		uint32_t ms2 = to_ms_since_boot(get_absolute_time());
		if (ms2 != ms)
		{
			// 1ms tick
			ms = ms2;
